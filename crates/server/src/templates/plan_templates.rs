...
pub(crate) fn import_picker_modal(
    ...
    html! {
        ...
        div class="bg-paper rounded-lg shadow-xl w-full max-w-md pointer-events-auto p-6" {
            h2 class="font-serif-heading text-lg font-medium mb-1" style="color: var(--ink)" { "Use a template" }
            ...
            @if templates.is_empty() {
                p class="text-sm py-4" style="color: var(--muted)" { "No templates available. Create one in Admin → Plan Templates." }
            } @else {
                div class="space-y-1 max-h-64 overflow-y-auto" {
                    @for tmpl in templates {
                        ...
                        form method="post" action=(format!("/practices/{}/import-template", practice_id))
                            ...
                            button type="submit"
                                   class="w-full text-left rounded px-3 py-2 hover:opacity-80 cursor-pointer"
